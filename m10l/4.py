...
def maximoLista(max, indice, lista):
    if indice < len(lista):
        if max == 0 or lista[indice] > max:
            max = lista[indice]
            
        return maximoLista(max, indice+1, lista)
    return max

def listaNNumeros(max, n, lista):
    if n >= 0:
        if max == 0 or lista[n] > max:
            max = lista[n]

        return listaNNumeros(max, n-1, lista)
    return max

def buscarCaracter(string, carac, indice):
    ...
    while True:
        ...
        try:
            if opcoes == "1":
                ...
                print(f"O máximo da lista inserida é: {maximoLista(maximo, indice, lista)}")
                ...
                print(f"O máximo da lista inserida é: {listaNNumeros(maximo, n-1, lista)}")

            elif opcoes == "3":
                s = input("Insira uma palavra: ")
                c = input("Insira um caractere para ser buscado na palavra anterior: ")
                print(f"O caractere {c} está no index: {buscarCaracter(s, c, indice)}")
